...
def add_student():
    ...
    if conn:
        try:
            ...
            major = input("Enter major ID: ")
            query = """
            INSERT INTO student (username, password, role, fname, lname, major) VALUES (%s, %s, %s, %s, %s, %s)
            """
            cursor.execute(query, (username, password, role, fname, lname, major))
            conn.commit()
            cursor.close()
